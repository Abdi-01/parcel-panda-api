        ...
        try {
            ...
            if (role === 'admin') {
                ...
                if (req.query.from && req.query.to) {
                    ...
                    // queryTopParcelRevenue = `SELECT idparcel_type AS parcel, (parcel_type.price - SUM(product.price)) AS profit FROM db_parcelpanda.transaction_detail JOIN parcel_type ON transaction_detail.idparcel_type = parcel_type.id JOIN product ON transaction_detail.idproduct = product.id WHERE date_transaction BETWEEN ${db.escape(req.query.from)} AND ${db.escape(req.query.to)} GROUP BY idparcel_type ORDER BY profit DESC`
                }
